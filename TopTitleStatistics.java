...
// <<< Don't Change
    ...
    public static class TopTitlesStatReduce extends Reducer<NullWritable, TextArrayWritable, Text, IntWritable> {
        ...
        public void reduce(NullWritable key, Iterable<TextArrayWritable> values, Context context) throws IOException, InterruptedException {
            ...
            for (Pair<Integer, String> item : countToWordMap)
            {
            	tmpSum += item.first.intValue();
            }
            sum = Integer.valueOf(tmpSum);
            
            int tmpMean = tmpSum / countToWordMap.size();
            mean = Integer.valueOf(tmpMean);
            
            int tmpSumOfSquaredDiffs = 0;
            for (Pair<Integer, String> item : countToWordMap)
            {
            	tmpSumOfSquaredDiffs += (int)Math.pow((double)(item.first.intValue() - tmpMean), 2.0);
            }
            var = Integer.valueOf(tmpSumOfSquaredDiffs/countToWordMap.size());
